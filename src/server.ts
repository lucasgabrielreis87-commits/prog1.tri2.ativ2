...
const server = Bun.serve({
    ...
    routes: {
        ...
        '/todo/:index': {
            GET: (req) => {
                return new Response("bhruu", { status: 501 })
            },
            DELETE: async (req) => {
                const strIndex = req.params.index
                const index = parseInt(strIndex)
                if (isNaN(index))
                    return new Response('/todo/:index index precisa ser um número', { status: 400 })
                try {
            ...
            PUT: async (req) => {
                ...
                try {
                    ...
                    return new Response('Erro ao atualizar ITEM:', { status: 500 })
                }
